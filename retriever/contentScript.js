...
window.onload = function () {
  ...
  const tooltip = new TextTip({
    ...
    buttons: [
      {
        title: "yellow-light",
        icon: chrome.runtime.getURL("assets/colors/yellow-light.svg"),
        callback: success,
      },
      {
        title: "green-light",
        icon: chrome.runtime.getURL("assets/colors/green-light.svg"),
        callback: success,
      },
      {
        title: "blue-light",
        icon: chrome.runtime.getURL("assets/colors/blue-light.svg"),
        callback: success,
      },
      {
        title: "pink-light",
        icon: chrome.runtime.getURL("assets/colors/pink-light.svg"),
        callback: success,
      },
    ],
  });
...
function addToStorage(sel, range, color) {
  ...
  if (range.commonAncestorContainer.nodeName == "#text") {
    dataArray["grandParentNodeName"] = range.commonAncestorContainer.parentNode.nodeName;
    dataArray["grandParentHTML"] = range.commonAncestorContainer.parentNode.innerText;
...
function restoreHighlight(nodeInfo) {
  ...
  if (nodeInfo.ancestorNodeName != "#text") {
    commonAncestorContainer = findAncestor(nodeInfo.ancestorNodeName, nodeInfo.ancestorHTML);
  } else {
    commonAncestorContainer = findAncestor(nodeInfo.grandParentNodeName, nodeInfo.grandParentHTML);
  }
...
function findAncestor(tagName, innerHTML) {
  ...
  for (let i = 0; i < lists.length; i++) {
    if (lists[i].innerText == innerHTML || lists[i].innerText.includes(innerHTML)) {
      return lists[i];
    }
  }
}
